...
public:
  ...
  public:
    ...
    DomainNameNode(std::string key, void *payload, int order, bool is_wild, time_t load_time)
      : key(key), order(order), payload(payload), parent(NULL), is_wild(is_wild), load_time(load_time)
    {
    }
  ...
  DomainNameNode *remove(std::string key);
  int expire(int);
  int dump(void);
