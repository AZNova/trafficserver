...
ClassAllocator<ProxyProtocolClientSession> proxyprotocolClientSessionAllocator("proxyClientSessionAllocator");
...
ProxyProtocolClientSession *
ProxyProtocolClientSession::alloc()
{
  return proxyprotocolClientSessionAllocator.alloc();
}
...
ProxyProtocolClientSession::new_connection(NetVConnection *new_vc, MIOBuffer *iobuf, IOBufferReader *reader, bool backdoor)
{
  ...
  // PROXY for the backdoor connections? Let's not deal woth that yet.
  ink_release_assert(backdoor == false);

  //this->init(new_vc);

  this->req_buffer = iobuf ? reinterpret_cast<TSIOBuffer>(iobuf) : reinterpret_cast<TSIOBuffer>(new_empty_MIOBuffer());
  //this->req_reader = reader ? reinterpret_cast<TSIOBufferReader>(reader) : TSIOBufferReaderAlloc(this->req_buffer);

  this->resp_buffer = reinterpret_cast<TSIOBuffer>(new_empty_MIOBuffer());
  //this->resp_reader = TSIOBufferReaderAlloc(this->resp_buffer);

  this->client_vc = new_vc;
  this->client_vc->set_tcp_congestion_control(CLIENT_SIDE);

  //EThread *thread(this_ethread());
  //MUTEX_TAKE_LOCK(this->mutex, thread);
...
//
//  if (this->sm_reader->read_avail() >= (int64_t)PROXY_V1_CONNECTION_PREFACE_LEN) {
